...
def main():
    ...
    for u in users:
        ...
        conn.execute(
            "INSERT INTO users (id,name,email,password_hash,pin_hash,role) VALUES (?,?,?,?,?,?)",
            (u["id"], u["name"], u["email"], fake_hash("demo-passwort"), fake_hash(u["pin"]), u["role"]),
        )
    ...
    for od in orders_data:
        cust_id = uid()
        conn.execute("INSERT INTO customers (id,name) VALUES (?,?)", (cust_id, od["customer"]))
        ...
        order_id = uid()
        conn.execute(
            """INSERT INTO orders (id,order_number,vehicle_id,status,progress,
               estimated_duration_min,assigned_mechanic_id) VALUES (?,?,?,?,?,?,?)""",
            (order_id, order_num, veh_id, od["status"], od["progress"], od["est_min"],
    ...
    for name, _pct in locations:
        lid = uid()
        loc_ids[name] = lid
        conn.execute("INSERT INTO tire_locations (id,name,capacity) VALUES (?,?,8)", (lid, name))
