...
app = FastAPI(title="WerkstattFlow API", version="0.1.0")
...
def login(body: LoginRequest):
    """Schritt 1: Zugangsdaten des Werkstatt-Kontos prüfen."""
    with get_db() as db:
...
def verify_pin(body: PinRequest):
    """Schritt 2: PIN des gewählten Profils prüfen -> gibt Session-Token zurück."""
    with get_db() as db:
...
# =====================================================================
# Benutzerverwaltung (nur Admin darf Rollen ändern)
# =====================================================================

@app.get("/users")
def list_users(user: dict = Depends(get_current_user)):
    with get_db() as db:
        rows = db.execute("SELECT id, name, email, role, active FROM users ORDER BY name").fetchall()
...
def get_order(order_id: str, user: dict = Depends(get_current_user)):
    with get_db() as db:
        order = db.execute(ORDER_LIST_QUERY + " WHERE o.id = ?", (order_id,)).fetchone()
        if not order:
    ...
    with get_db() as db:
        rows = db.execute(
            """SELECT t.*, l.name AS location_name FROM tires t
               LEFT JOIN tire_locations l ON l.id = t.location_id ORDER BY t.name"""
        ).fetchall()
    return rows_to_list(rows)
...
def list_tire_locations(user: dict = Depends(get_current_user)):
    with get_db() as db:
        rows = db.execute("SELECT * FROM tire_locations ORDER BY name").fetchall()
    return rows_to_list(rows)
...
def list_parts(user: dict = Depends(get_current_user)):
    with get_db() as db:
        rows = db.execute("SELECT * FROM parts ORDER BY name").fetchall()
    return rows_to_list(rows)
...
def list_appointments(user: dict = Depends(get_current_user)):
    with get_db() as db:
        rows = db.execute("SELECT * FROM appointments ORDER BY scheduled_at").fetchall()
    return rows_to_list(rows)
