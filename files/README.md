...
## 7. Mehrere Werkstätten (Mandantentrennung) – bereits eingebaut ✅

Jeder Nutzer gehört zu genau einer `workshop_id`. Diese steckt im Login-Token
und wird bei **jeder** Abfrage automatisch als Filter angewendet – Werkstatt A
kann Daten von Werkstatt B weder sehen noch verändern, selbst wenn sie eine
exakte ID erraten/kennen (getestet: Cross-Tenant-Zugriff gibt `404`, nicht `403`,
damit ein Angreifer nicht mal erfährt, ob die ID woanders existiert).

Die Demo-Datenbank enthält zwei Werkstätten zum Testen:

| Werkstatt | Login | Passwort | PIN |
|---|---|---|---|
| Kabara Digital Pilot-Werkstatt (Hauptdaten) | mert.kabara@werkstattflow.de | demo-passwort | 1111 |
| Musterwerkstatt Test GmbH (nur zum Isolationstest, 1 Auftrag) | erika.musterfrau@testwerkstatt.de | demo-passwort | 9999 |

**Neue Werkstatt anlegen**: aktuell nur direkt in der Datenbank (kein Admin-UI dafür). In `seed_data.py` ist der Ablauf als Vorlage zu sehen: `INSERT INTO workshops`, dann Nutzer mit der neuen `workshop_id` anlegen.

## 8. Sicherheit – bereits eingebaut
...
## 9. Vor einem echten Piloten (mit Kollegen/echten Daten) noch offen
