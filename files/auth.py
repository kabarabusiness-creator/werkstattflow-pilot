...
def create_token(user_id: str, role: str, workshop_id: str) -> str:
    expire = datetime.datetime.utcnow() + datetime.timedelta(hours=TOKEN_EXPIRE_HOURS)
    payload = {"sub": user_id, "role": role, "workshop_id": workshop_id, "exp": expire}
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    return {"id": payload["sub"], "role": payload["role"], "workshop_id": payload.get("workshop_id")}
